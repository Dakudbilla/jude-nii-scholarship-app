      ...
      {!statusMutation.data ? (
        <div className="bg-white p-8 sm:p-12 rounded-3xl border border-slate-100 shadow-[0_20px_60px_rgb(0,0,0,0.04)] max-w-md w-full relative">
          ...
          >
            ...
            <div className="pt-2">
              <Button
                type="submit"
                className="w-full h-14 rounded-xl font-bold text-lg bg-primary hover:bg-primary/90 text-white shadow-xl shadow-primary/20"
        ...
        <div className="bg-white p-8 sm:p-12 rounded-3xl border border-slate-100 shadow-[0_20px_60px_rgb(0,0,0,0.06)] max-w-lg w-full text-center space-y-8 relative overflow-hidden">
              ...
              {statusMutation.data.status && (() => {
                ...
                return (
                  <div className="pt-2">
                    ...
                    <div className={`px-6 py-4 rounded-2xl text-lg font-bold shadow-sm ${cfg.badgeClass}`}>
                      {cfg.label}
                    </div>
                    <p className="text-slate-500 text-sm mt-3 font-medium">{cfg.description}</p>
                  </div>
                );
              })()}
            </div>
              ...
              <div className="flex gap-4 items-start">
                ...
                <div>
                  ...
                  <p className="text-amber-800/80 font-medium leading-relaxed">
                    We couldn&apos;t find a submitted application in the active cycle for these credentials.
                    If you started a draft, please return to the Application Portal to complete and submit it.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button variant="ghost" className="h-12 px-6 font-bold text-slate-500 hover:text-slate-800 rounded-xl border border-slate-200" onClick={() => statusMutation.reset()}>
              Check Another
            </Button>
            <Button variant="ghost" className="h-12 px-6 font-bold text-slate-500 hover:text-slate-800 rounded-xl border border-slate-200" onClick={() => window.location.href = "/"}>
              ← Home
            </Button>
          </div>
        </div>
